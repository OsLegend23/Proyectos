...
{		enableBasicTinyMCE('textarea', 950);	setCurrentState(STATE_SAVE);	$("#tblRecorded").flexigrid({
			url: '<?php   echo  $viewpage.'/'; ?>remote.php',			dataType: 'json',						params:[{name:'opt', value: 'getList'}],			singleSelect:true,			colModel : [							{display: '<?php   echo  $STR['Edit'];  ?>', name : 'edit', width : 80, sortable : false, align: 'center'},
				{display: '<?php   echo  $STR['Delete'];  ?>', name : 'delete', width : 80, sortable : false, align: 'center'},								{display: '<?php   echo  $STR['KnowledgeName'];  ?>', name : 'knowledgename', width :460, sortable : false, align: 'center'},				{display: '<?php   echo  $STR['Domain'];  ?>', name : 'domain', width : 280, sortable : false, align: 'center'}				],
			sortname: "",			sortorder: "",			usepager: false,			title: '<?php   echo  $STR['InformaticList'];  ?>',			useRp: false,			rp: 15,
...
}function setRecordInfo(data){		$('#KnowledgeName').val(data.knowledgename);	$('#Domain').val(data.domain);
	if(!isBlank(data.description))		$('#Description').val(data.description);	$('#btnSubmit').focus();}
function showRequest(formData, jqForm, options) {   		openOverlay();	displayPrompt('#'+tagId, '', 'wait_progress', false);    return true; 
} function showResponse(data)  { 
	closeOverlay();	if(isBlank(data))	{		displayPrompt('#'+tagId, '<?php   echo  $STR['Msg_WebError']; ?>', 'error', false);		return false;
	}	if(data.state == STATE_GETRECORDINFO)	{		setCurrentState(STATE_UPDATE);		setRecordInfo(data);
		closePromt('#'+tagId);	}	if(data.state == STATE_SAVE)	{		if(data.answer == 'fail')
		{		}		else if(data.answer = 'correct')		{			setCurrentState(STATE_SAVE);
