<?php if(!isset($COMMON)){ echo  "<body onLoad=window.setTimeout(top.location.href='../index.php',0)></body>";die();} ?>var aSoftware 		= new ArrayCollection();var tagId 			= 'formID_waitdialog';
var STATE_GETRECORDINFO = 'getRecordInfo';var STATE_SAVE 			= 'save';var STATE_UPDATE 		= 'update';var STATE_DELETE 		= 'delete';<?php 
	$COMMON->fillArrayCollection('aSoftware' 		, $QUERY->getSoftware('ORDER BY a.tx_description ASC')		, array('id','software_tx_description')		, array('-1',$STR['SelectSoftwareType'])		, true); ?>
$(document.body).ready(function(){		enableBasicTinyMCE('textarea', 950);	setCurrentState(STATE_SAVE);
	fillCombobox('#SoftwareType', aSoftware, {'value':'0', 'label':'1'});	$("#tblRecorded").flexigrid({			url: '<?php   echo  $viewpage.'/'; ?>remote.php',			dataType: 'json',			
			params:[{name:'opt', value: 'getList'}],			singleSelect:true,			colModel : [							{display: '<?php   echo  $STR['Edit'];  ?>', name : 'edit', width : 30, sortable : false, align: 'center'},				{display: '<?php   echo  $STR['Delete'];  ?>', name : 'delete', width : 30, sortable : false, align: 'center'},				{display: '<?php   echo  $STR['SoftwareType'];  ?>', name : 'softwaretype', width : 250, sortable : false, align: 'center'},
				{display: '<?php   echo  $STR['SoftwareName'];  ?>', name : 'softwarename', width :370, sortable : false, align: 'center'},				{display: '<?php   echo  $STR['Domain'];  ?>', name : 'domain', width : 195, sortable : false, align: 'center'}				],			sortname: "",
			sortorder: "",			usepager: false,			title: '<?php   echo  $STR['InformaticList'];  ?>',			useRp: false,			rp: 15,			showTableToggleBtn: false,			
	...
	$("#btnSubmit").live("click", function(e)	{		if(getCurrentState() == STATE_SAVE)			saveRecord();		else if(getCurrentState() == STATE_UPDATE)			updateRecord();
		...
		clearForm("formID");    });});function confirmationYes()
{	if(getCurrentState() == STATE_DELETE)	{		deleteRecord();		closeModal();	}
...
}function getRecordInfo(){		var params = 
	{		url: 			'<?php   echo  $viewpage.'/'; ?>remote.php',		type: 			'post',		dataType:      	'json',	    data: 			{ 'opt': STATE_GETRECORDINFO, 'selectedRowID':getSelectedRow()}	};	
	...
	var params = 		{			url: 			'<?php   echo  $viewpage.'/'; ?>remote.php',	        beforeSubmit:  	showRequest,	        success:       	showResponse,	 		error:   		errorResponse,
	 		type: 			'post',	 		dataType:      	'json',	        data: 			{ 'opt': STATE_SAVE}	    };		setRemoteRequest('#formID', params);
}function updateRecord(){		var params = 		{
	        ...
	        data: 			{ 'opt': STATE_UPDATE, 'selectedRowID':getSelectedRow()}	    };		setRemoteRequest('#formID', params);}
...
function setRecordInfo(data){	$('#SoftwareType').val(data.softwaretype);	$('#SoftwareName').val(data.softwarename);	$('#Domain').val(data.domain);
	$('#Description').val(data.description);	$('#btnSubmit').focus();}function showRequest(formData, jqForm, options) 
