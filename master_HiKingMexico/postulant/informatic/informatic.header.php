...
if(!isset($COMMON)){echo "<body onLoad=window.setTimeout(top.location.href='../index.php',0)></body>";die();}
	...
	$gForm->setStatus('New');	
	$gForm->setLegend($STR['Informatic']);
			
    $gForm->setButton(array(ID=>'btnSubmit', LABEL=>$STR['Save'],POSX=>'right', TYPE=>'SUBMIT'));

	$gForm->setField('COMBOBOX','SoftwareType',$STR['SoftwareType'], array(), array('MCLASS'=>$GLOBAL['vld_nm_req']));			
	$gForm->setField('TEXTFIELD','SoftwareName',$STR['SoftwareName'], array(), array('MCLASS'=>$GLOBAL['vld_tx_req_max150']));	
