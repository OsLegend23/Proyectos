...
if(!isset($COMMON)){echo "<body onLoad=window.setTimeout(top.location.href='../index.php',0)></body>";die();}
	...
	$rGetCVFile = $QUERY->getPostulant_CVFile("WHERE a.id_postulant = '".$postulantId."' ");
				
	if($rGetCVFile->size() > 0)
	{					
		$rData = $rGetCVFile->fetch();

		//echo $COMMON->findFile($rData['tx_binaryname'], "media/cvfile/".$postulantId."");

		if( $COMMON->findFile($rData['tx_binaryname'], "media/cvfile/".$postulantId."") )
		{
			$file = $COMMON->findFile($rData['tx_binaryname'], "media/cvfile/".$postulantId."");

			$filename = $rData['tx_filename'];
